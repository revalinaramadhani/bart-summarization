...
class StreamlitUI:
    ...
    def render_halaman_utama(self):
        ...
        st.file_uploader(
            ...
            key="pdf_file_uploader"  # <--- INI KUNCI UTAMANYA
            # Hapus 'args'
        )
        
        # Hapus semua kode ini, sudah tidak diperlukan lagi
    ...
    def render_halaman_utama(self):
        ...
        file_pdf = st.file_uploader(
            "Upload PDF", 
            type=["pdf"],  # Filter otomatis hanya .pdf (Sesuai Skenario IV-8)
            on_change=self._handle_upload_pdf, 
            args=(st.session_state.get('file_uploader_key'),) # Trik agar on_change terpicu
        )
        # Trik untuk Streamlit file_uploader
        if 'file_uploader_key' not in st.session_state:
            st.session_state.file_uploader_key = 0
        if file_pdf:
            st.session_state.file_uploader_key += 1
